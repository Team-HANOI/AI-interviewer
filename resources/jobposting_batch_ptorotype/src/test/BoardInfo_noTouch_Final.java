...
//					tags.get("name")
				...
				try {
					...
					String sql = "insert into job_posting(company,url,logourl,stacks,jobposition) values( ? , ? , ? , ? , ? )";
					// where 로 select 해서 url 비교 없으면 삽입 있으면 업데이트
					...
					pstmt.setString(5, title.get("title").toString());
